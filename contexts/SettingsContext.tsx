...
const translations = {
  tr: {
    ...
    clearScientificHistoryDesc:
      "Bilimsel hesap makinesi geçmişini temizlemek istediğinizden emin misiniz?",
    ...
    // KDV Hesaplayıcı
    kdv: "KDV",
    kdvCalculator: "KDV Hesaplayıcı",
    priceWithoutKdv: "KDV Hariç Fiyat",
    kdvRate: "KDV Oranı",
    kdvAmount: "KDV Tutarı",
    totalWithKdv: "KDV Dahil Toplam",
    ...
    tipPercentage: "Bahşiş Yüzdesi (%)",
    numberOfPeople: "Kişi Sayısı",
    tipAmount: "Bahşiş Tutarı",
    totalWithTip: "Bahşiş Dahil Toplam",
    perPerson: "Kişi Başı",

    // Yaş Hesaplayıcı
    age: "Yaş",
    ageCalculator: "Yaş Hesaplayıcı",
    birthDate: "Doğum Tarihi",
    enterBirthDate: "Doğum Tarihi (GG.AA.YYYY)",
    yourAge: "Yaşınız",
    nextBirthday: "Sonraki Doğum Günü",
    daysUntilBirthday: "Doğum gününüze kalan",
  },
  en: {
    // Calculator
    ...
    clearScientificHistoryDesc:
      "Are you sure you want to clear the scientific calculator history?",
    ...
    // VAT Calculator
    kdv: "VAT",
    kdvCalculator: "VAT Calculator",
    priceWithoutKdv: "Price Without VAT",
    kdvRate: "VAT Rate",
    kdvAmount: "VAT Amount",
    totalWithKdv: "Total With VAT",
    ...
    enterBirthDate: "Birth Date (DD.MM.YYYY)",
    yourAge: "Your Age",
...
export function SettingsProvider({ children }: SettingsProviderProps) {
  ...
  const [history, setHistory] = useState<
    { calculation: string; result: string; timestamp: Date; note?: string }[]
  >([]);
    ...
    try {
      ...
      if (storedSettings) {
        ...
        setSettings({
          ...
          showDecimals:
            parsedSettings.showDecimals ?? defaultSettings.showDecimals,
          vibrationEnabled:
            parsedSettings.vibrationEnabled ?? defaultSettings.vibrationEnabled,
          soundEnabled:
            parsedSettings.soundEnabled ?? defaultSettings.soundEnabled,
          defaultAngleUnit:
            parsedSettings.defaultAngleUnit ?? defaultSettings.defaultAngleUnit,
          language: parsedSettings.language ?? defaultSettings.language,
          highContrast:
            parsedSettings.highContrast ?? defaultSettings.highContrast,
  ...
  const loadScientificHistory = async () => {
    try {
      const storedScientificHistory = await AsyncStorage.getItem(
        "calculator_scientific_history",
      );
  ...
  const addToHistory = async (calculation: string, result: string) => {
    ...
    const updatedHistory = [newEntry, ...history.slice(0, 49)]; // Son 50 işlemi sakla
  ...
  const addToScientificHistory = async (
    calculation: string,
    result: string,
  ) => {
    const newEntry = { calculation, result, timestamp: new Date() };
    const updatedHistory = [newEntry, ...scientificHistory.slice(0, 49)]; // Son 50 işlemi sakla
